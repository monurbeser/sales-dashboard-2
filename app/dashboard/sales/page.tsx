...
export default function SalesDashboard() {
  ...
  const { data: analytics, isLoading } = useQuery({
    queryKey: ['salesAnalytics', selectedManager],
    queryFn: async () => {
      if (!selectedManager) return null;
      const res = await fetch(`/api/analytics/sales/${selectedManager}?year=2026`);
  ...
  if (managers.length === 0) {
    return <div className="flex h-[50vh] items-center justify-center text-slate-500">Kayıtlı satış yöneticisi bulunmuyor.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Kişisel Dashboard (Satış)</h1>
          <p className="text-muted-foreground">Satış yöneticisi bazlı hedef ve gerçekleşmeler.</p>
        </div>
        ...
        <>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="border-l-4 border-l-emerald-500 shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-slate-600">Toplam Satış</CardTitle>
            ...
            <Card className="border-l-4 border-l-indigo-500 shadow-sm">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium text-slate-600">Fatura Sayısı</CardTitle>
          ...
          <div className="grid gap-6 grid-cols-1 lg:grid-cols-4">
            <Card className="lg:col-span-3 shadow-sm border-slate-200">
              <CardHeader>
                <CardTitle className="text-lg">Kişisel Aylık Hedef vs Gerçekleşme (₺)</CardTitle>
              </CardHeader>
              <CardContent className="h-[380px] w-full pt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={analytics.monthlyData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                    ...
                    <Bar dataKey="gerceklesme" name="Kişisel Satış" fill="#3B82F6" radius={[4, 4, 0, 0]} maxBarSize={40} />
                  </ComposedChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
                ...
                {analytics.quarters.map((q: any) => (
                  <div key={q.name} className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs font-mono text-slate-500">
                      ...
                      <span>S: {formatTLCompact(q.actual)}</span>
                    </div>
                  </div>
                ))}
              </CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>İlgili Ürün Yöneticisi</TableHead>
